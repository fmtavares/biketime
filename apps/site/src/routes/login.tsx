...
/**
 * Login do portal do cliente (somente e-mail + senha).
...
function LoginClientePage() {
   ...
   */
  async function clienteVinculado(userId: string) {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      const ok = await clienteVinculado(uid);
      if (!ok) {
        const { data: staff } = await (supabase as any).rpc("is_staff", { _user_id: uid });
    ...
    <div className="container-px mx-auto flex min-h-[70vh] max-w-md flex-col justify-center py-16">
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Entre com o e-mail e a senha fornecidos pela Bike Time.
        </p>
      </div>
