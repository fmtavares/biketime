/**
 * Transfere fotos da Sava Carbon (estoque) do Lovable → Site Bike Time
 * e atualiza as URLs públicas em bikes_estoque.
 *
 * Uso: node scripts/transfer-estoque-fotos-sava.mjs
 */
...
const BIKES = [
  {
    id: "7be0d58c-b81e-43eb-8404-06d68d48a8d9",
    label: "Sava Carbon MTB",
  },
];
...
async function main() {
  ...
  for (const bike of BIKES) {
    ...
    if (!files?.length) {
      console.log("  origem vazia — tentando baixar pelas URLs do destino…");
      // Fallback: baixa pelas URLs públicas antigas já gravadas no destino
      const { data: row } = await dst
        .from("bikes_estoque")
        .select(URL_FIELDS.join(","))
        .eq("id", bike.id)
        .maybeSingle();
      if (!row) {
        console.error("  bike não encontrada");
        fail++;
        continue;
      }
      ...
      for (const field of URL_FIELDS) {
        ...
        try {
          ...
          patch[field] = `${dstPublicBase}/${path}`;
